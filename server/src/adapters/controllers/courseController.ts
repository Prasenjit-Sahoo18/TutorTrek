...
const courseController = (
  ...
  const getDiscussionsByLesson = asyncHandler(
    async (req: Request, res: Response) => {
      const lessonId = req.params.lessonId;
      const discussions = await getDiscussionsByLessonU(
        lessonId,
        dbRepositoryDiscussion
      );
      res.status(200).json({
  ...
  const editDiscussions = asyncHandler(
    async (req: CustomRequest, res: Response) => {
      ...
      res.status(200).json({
        status: 'success',
        message: 'Successfully updated the discussion',
        data: null
      });
    }
  );
  ...
  const replyDiscussion = asyncHandler(
    async (req: CustomRequest, res: Response) => {
      const studentId = req.user?.Id;
      const discussionId = req.params.discussionId;
      const reply: AddDiscussionInterface = req.body;
      if (!studentId) {
        throw new Error('Student ID is required');
      }
      await replyDiscussionU(
        discussionId,
        { studentId, message: reply.message },
        dbRepositoryDiscussion
      );
      res.status(200).json({
        status: 'success',
        message: 'Successfully added reply to the discussion',
        data: null
      });
    }
  );
  ...
  const getCourseByStudent = asyncHandler(
    async (req: CustomRequest, res: Response) => {
      ...
      res.status(200).json({
        status: 'success',
        message: 'Successfully retrieved courses based on the student',
        data: courses
      });
    }
  );
  ...
  const searchCourse = asyncHandler(async (req: Request, res: Response) => {
    ...
    const courses = await searchCourseU(
      searchQuery,
      filterQuery,
      cloudService,
      dbRepositoryCourse
